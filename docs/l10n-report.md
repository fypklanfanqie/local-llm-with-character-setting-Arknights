...
| `llm/` 诊断层（额外批次 11） | 164 条中 **59 条已覆盖（36%）**，其余为插值/提示词 |
...
1. **指南搜索按中文匹配**：标题/别名是数据层中文，英/日关键词搜不到中文标题（ASCII 别名如 deepseek/group/tts/npu 可命中）。
2. **`llm/` 诊断层文案已适配 36%**（164 条中 59 条进词典、35 条为日志）：后端名称与说明、后端偏好、NPU 芯片等级与理由、推理性能模式、温度状态、后端健康决策理由、静态基准否决原因、基准场景/四象限名称已随语言切换。**仍未翻译的 105 条**为：基准/思考层的**含插值否决原因**（如 `TTFT 劣化超 30%（候选=$ct vs 基线=$bt）`，需把数据层改成模板才能翻译）、`ModelBundleValidator` 的路径/分片校验（含插值）、`MnnBackend` 内部错误消息、基准测试提示词、思考策略关键词表。
