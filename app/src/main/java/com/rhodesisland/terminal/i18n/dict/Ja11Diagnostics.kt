...
internal val Ja11DiagnosticsEntries: List<Pair<String, String>> = listOf(
    ...
    // ===== NPU の理由（データ層をテンプレート化）=====
    "支持 NPU: {0}" to "NPU 対応：{0}",
    "非高通设备 ({0}), NPU 后端仅支持骁龙芯片" to
        "Qualcomm 以外の端末（{0}）。NPU バックエンドは Snapdragon のみ対応",
    "标准构建不含 QNN 运行时，NPU 不可用（QNN 仅实验 flavor 支持）" to
        "標準ビルドには QNN ランタイムが含まれないため NPU は利用できません（QNN は実験的フレーバーのみ）",

    // ===== GPU ウォームアップの理由（データ層をテンプレート化）=====
    "OpenCL 健康检查未通过（{0}），未执行预热" to
        "OpenCL のヘルスチェックに合格しませんでした（{0}）。ウォームアップは実行していません",
    "GPU 预热未生效：实际走了 {0}（OpenCL 加载/生成失败回退；耗 {1}ms）" to
        "GPU ウォームアップが効いていません：実際は {0} で実行（OpenCL の読み込み/生成失敗によるフォールバック、{1}ms）",

    // ===== 思考戦略テレメトリ =====
    "旧两阶段记录兼容；新记录恒为 0" to "旧 2 段階記録との互換用。新しい記録では常に 0",
