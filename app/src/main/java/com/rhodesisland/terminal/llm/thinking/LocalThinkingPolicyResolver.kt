...
class LocalThinkingPolicyResolver {
     ...
     *
     * 语义关键词（「复杂」「证明」等）最多加 1 分，且不能单独把问题判为 COMPLEX—— // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
    ...
    private fun buildInstruction(effectiveLevel: LocalThinkingLevel, checkpointBudget: Int): String {
        val depthHint = when (effectiveLevel) {
            LocalThinkingLevel.SHORT -> "多数问题凭直觉直接给结论" // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            LocalThinkingLevel.MEDIUM -> "检查关键假设与明显反例后再下结论" // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            LocalThinkingLevel.LONG -> "复杂处可多列几条要点，覆盖边界与自检" // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            LocalThinkingLevel.AUTO -> "按问题需要取舍深浅" // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
        }
        return "\n\n【思考要求（仅本轮生效）】\n" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            "本提示只约束你的内部思考过程，不改变最终回答的格式、篇幅与内容要求。\n" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            "用以下思考模板组织思考，每行直给内容：\n" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            "目标：<一句话写清本次要解决什么>\n" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            "要点：<最多 $checkpointBudget 条，每条一行；禁止：复述问题原文、重复验证已确认的步骤>" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            "（$depthHint）\n" +
            "结论：<据此作答的最短路径>\n" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
            "满足上述模板即可立即停止思考并写出完整最终答案；不得因思考简短而省略重要结论。" // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
    }
    ...
    private companion object {
        ...
        val KEYWORDS = listOf("证明", "调试", "架构", "比较", "推导", "修复") // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
        ...
        val TRIVIAL_CASUAL = Regex(
            "(?:(?:你好|您好|哈喽|嗨+|嗨呀|hi+|hello+|hey+|yo+|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "早安|早上好|中午好|下午好|晚上好|晚安|安|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "在吗|在么|在不在|吃了吗|睡了吗|醒了吗|忙吗|忙不忙|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "谢谢(?:你|啦|了)?|感谢(?:你)?|多谢|麻烦(?:你)?了|辛苦(?:你)?[了啦]|有劳了|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "好的|好吧|好呀|好嘞|好哒|好耶|好滴|可以(?:的)?|行的|行吧|中|成(?:吧)?|没问题|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "嗯+|哦+|噢+|喔+|诶+|欸+|呃+|唔+|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "ok(?:ay)?|收到|了解|明白|知道(?:了)?|懂(?:了)?|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "拜拜|再见|回见|明天见|下次见|回头见|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "抱抱|亲亲|摸摸头|揉揉|拍拍|贴贴|比心|爱你|想你|想你了|喜欢你们?|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "哈哈+|呵呵+|嘿嘿+|嘻嘻+|嘶+|呼+|呜呜+|哇+|哇哦|哦哦+|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
                "厉害[了呀啊]?|牛[逼b][呀啊]?|太棒了|真棒|赞(?:一个)?|强[呀啊]?|" + // l10n:ignore 非界面文案（提示词/正则/内部消息，仅日志或经映射）
